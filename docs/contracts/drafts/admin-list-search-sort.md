...
| `sort` | `field,asc\|desc` | 단일 필드만 지정한다. 방향 생략 시 `asc`. 카테고리별 허용 필드 목록에 없으면 `400 COMMON_VALIDATION_FAILED` |

두 파라미터 모두 생략하면 카테고리별 기본 정렬을 그대로 사용한다. 페이지네이션은 이번 규정에 포함하지 않는다 - 현재 데이터 규모에서는 검색으로 충분히 좁혀지고, 필요해지면 별도로 정의한다.
