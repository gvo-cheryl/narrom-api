...
JWT 필터처럼 Controller 이전에 발생한 인증 실패를 `ErrorCode`와 함께 전달한다.

- Spring Security의 `AuthenticationException` 계열
- `AUTH_REQUIRED`, `AUTH_ACCESS_TOKEN_EXPIRED`, `AUTH_ACCESS_TOKEN_INVALID` 등에 사용
- 응답 생성은 Exception 자체가 아니라 `ApiAuthenticationEntryPoint`가 담당
...
| JWT 필터 | 토큰 없음·만료·위조 | `ApiAuthenticationException` | `ApiAuthenticationEntryPoint` |
...
커스텀 JWT 필터가 `ExceptionTranslationFilter`보다 앞에서 실행되면 필터 내부에서 인증 예외를 잡아 `ApiAuthenticationEntryPoint`에 위임한다. 필터가 직접 임의 JSON을 작성하지 않는다.
