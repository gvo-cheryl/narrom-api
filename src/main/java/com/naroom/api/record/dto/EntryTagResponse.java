...
public record EntryTagResponse(UUID id, TagResponse tag, TagSource source, TagState state) {

	public static EntryTagResponse from(EntryTag entryTag) {
		return new EntryTagResponse(
				entryTag.getId(), TagResponse.from(entryTag.getTag()), entryTag.getSource(), entryTag.getState());
	}
